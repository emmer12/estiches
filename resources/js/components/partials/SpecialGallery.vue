    ...
    <div class="portfolio">
            ...
            <div class="grid">
                <isotope ref='cpt' :options='option' :list="urls"   v-images-loaded:on.progress="layout" class="wow fadeIn" @filter="filterOption=arguments[0]" :item-selector="'element-item'" id="root_isotope">
                    <div   v-for="element in galleries" :key="element.id" class="grid-items" style="padding:10px;box-shadow: 2px 3px 4px #ccc;margin: 5px;">
                        <div class="grid-sizer"></div> 
                        <!-- <LightBox :media="[{src:element.img}]"></LightBox> -->
                        <img  data-toggle="modal" :data-target="'#modelId'+element.id" v-if="element.category!='Video'" :src="'/storage/uploads/images/'+element.filename" alt="" width="100%">
                          ...
                          <div class="modal-dialog" role="document">
                            <div class="modal-content">
                              ...
                              <div class="modal-body">
                               <img :src="'/storage/uploads/images/'+element.filename" :alt="element.category" width="100%">
                              </div>
                            </div>
                          </div>
                        </div>

                        <a :href="'/storage/uploads/images/'+element.filename" download class="download">
...
export default {
  ...
  directives: {
    imagesLoaded,
        ...
        computed: {
           galleries(){
            return this.$store.getters.gallery.filter(gallery=>gallery.category=="Special")
          },
        },
}
</script>
